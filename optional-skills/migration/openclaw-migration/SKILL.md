...
description: Migrate a user's OpenClaw customization footprint into Ov3rwatch Agent. Imports Ov3rwatch-compatible memories, SOUL.md, command allowlists, user skills, and selected workspace assets from ~/.openclaw, then reports exactly what could not be migrated and why.
version: 1.0.0
author: Ov3rwatch Agent (Nous Research)
license: MIT
metadata:
  hermes:
    tags: [Migration, OpenClaw, Ov3rwatch, Memory, Persona, Import]
    related_skills: [hermes-agent]
---

# OpenClaw -> Ov3rwatch Migration

Use this skill when a user wants to move their OpenClaw setup into Ov3rwatch Agent with minimal manual cleanup.
...
- import `SOUL.md` into the Ov3rwatch home directory as `SOUL.md`
- transform OpenClaw `MEMORY.md` and `USER.md` into Ov3rwatch memory entries
- merge OpenClaw command approval patterns into Ov3rwatch `command_allowlist`
- migrate Ov3rwatch-compatible messaging settings such as `TELEGRAM_ALLOWED_USERS` and `MESSAGING_CWD`
- copy OpenClaw skills into `~/.hermes/skills/openclaw-imports/`
- optionally copy the OpenClaw workspace instructions file into a chosen Ov3rwatch workspace
- mirror compatible workspace assets such as `workspace/tts/` into `~/.hermes/tts/`
- archive non-secret docs that do not have a direct Ov3rwatch destination
...
With `--migrate-secrets`, it will also import a small allowlisted set of Ov3rwatch-compatible secrets, currently:
...
Ov3rwatch CLI supports the `clarify` tool for interactive prompts, but it is limited to:
...
- `{"question":"One or more imported OpenClaw skills already exist in Ov3rwatch. How should I handle those skill conflicts?","choices":["keep existing skills","overwrite conflicting skills with backup","import conflicting skills under renamed folders"]}`
...
- `{"question":"Do you want to copy the OpenClaw workspace instructions file into a Ov3rwatch workspace?","choices":["skip workspace instructions","copy to a workspace path","decide later"]}`
...
13. If a renamed imported skill has an empty `details.backup`, do not imply the existing Ov3rwatch skill was renamed or backed up. Say only that the imported copy was placed in the new destination and reference `details.renamed_from` as the pre-existing folder that remained in place.
...
2. Do not migrate secrets by default. Tokens, auth blobs, device credentials, and raw gateway config should stay out of Ov3rwatch unless the user explicitly asks for secret migration.
3. Do not silently overwrite non-empty Ov3rwatch targets unless the user explicitly wants that. The helper script will preserve backups when overwriting is enabled.
...
6. Even in secret-migration mode, only migrate secrets with a clean Ov3rwatch destination. Unsupported auth blobs must still be reported as skipped.
...
- Ov3rwatch persona state imported
- Ov3rwatch memory files populated with converted OpenClaw knowledge
