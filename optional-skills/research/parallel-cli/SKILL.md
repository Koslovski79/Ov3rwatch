...
This is an optional third-party workflow, not a Ov3rwatch core capability.

Important expectations:
- Parallel is a paid service with a free tier, not a fully free local tool.
- It overlaps with Ov3rwatch native `web_search` / `web_extract`, so do not prefer it by default for ordinary lookups.
...
Prefer Ov3rwatch native `web_search` / `web_extract` for quick one-off lookups when Parallel is not specifically requested.
...
7. Prefer Ov3rwatch native tools unless the user wants Parallel specifically or needs Parallel-only workflows.
...
Recommended Ov3rwatch workflow:
...
## Recommended Ov3rwatch usage patterns
...
- Do not silently choose Parallel when Ov3rwatch native tools are already sufficient.
