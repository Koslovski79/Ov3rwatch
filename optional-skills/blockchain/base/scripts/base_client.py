#!/usr/bin/env python3
"""
Base Blockchain CLI Tool for Ov3rwatch Agent
------------------------------------------
...
def _http_get_json(url: str, timeout: int = 10, retries: int = 2) -> Any:
    ...
    for attempt in range(retries + 1):
        req = urllib.request.Request(
            url, headers={"Accept": "application/json", "User-Agent": "Ov3rwatchAgent/1.0"},
        )
        try:
...
def _rpc_call(method: str, params: list = None, retries: int = 2) -> Any:
    ...
    _headers = {"Content-Type": "application/json", "User-Agent": "Ov3rwatchAgent/1.0"}
...
def _eth_call(to: str, selector: str, args: str = "", block: str = "latest") -> Optional[str]:
    ...
    try:
        ...
        req = urllib.request.Request(
            ...
            headers={"Content-Type": "application/json", "User-Agent": "Ov3rwatchAgent/1.0"},
            method="POST",
        )
        with urllib.request.urlopen(req, timeout=20) as resp:
            body = json.load(resp)
...
def main():
    parser = argparse.ArgumentParser(
        prog="base_client.py",
        description="Base blockchain query tool for Ov3rwatch Agent",
    )
