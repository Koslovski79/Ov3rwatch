---
name: telephony
description: Give Ov3rwatch phone capabilities without core tool changes. Provision and persist a Twilio number, send and receive SMS/MMS, make direct calls, and place AI-driven outbound calls through Bland.ai or Vapi.
version: 1.0.0
...
This optional skill gives Ov3rwatch practical phone capabilities while keeping telephony out of the core tool list.
...
It does **not** turn Ov3rwatch into a real-time inbound phone gateway. Inbound SMS is handled by polling the Twilio REST API. That is enough for many workflows, including notifications and some one-time-code retrieval, without adding core webhook infrastructure.
...
4. Treat third-party phone numbers as sensitive operational data:
   - do not save them to Ov3rwatch memory
...
### 1) "I want Ov3rwatch to own a real phone number"
Use **Twilio**.
...
- pairs well with Ov3rwatch `text_to_speech` plus a public file host or tunnel
...
Then save credentials into Ov3rwatch:
...
```bash
python3 "$SCRIPT" twilio-call "+15551230000" --message "Hello! This is Ov3rwatch calling with your status update." --voice Polly.Joanna
```

### E. Call with a prerecorded / custom voice message

This is the main path for reusing Ov3rwatch's existing `text_to_speech` support.

Use this when:
- you want the call to use Ov3rwatch's configured TTS voice rather than Twilio `<Say>`
...
Recommended Ov3rwatch TTS -> Twilio Play workflow:

1. Generate the audio with Ov3rwatch `text_to_speech`.
2. Make the resulting MP3 publicly reachable.
3. Place the Twilio call with `--audio-url`.

Example agent flow:
- Ask Ov3rwatch to create the message audio with `text_to_speech`
...
Important note:
- Ov3rwatch TTS is great for prerecorded outbound messages
...
- Ov3rwatch STT/TTS alone is not being used here as a full duplex phone conversation engine; that would require a much heavier streaming/webhook integration than this skill is trying to introduce
...
7. Summarize the outcome without persisting third-party numbers to Ov3rwatch memory.
...
- Do not store arbitrary third-party phone numbers in Ov3rwatch memory.
