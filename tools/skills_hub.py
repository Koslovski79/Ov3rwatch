#!/usr/bin/env python3
"""
Skills Hub — Source adapters and hub state management for the Ov3rwatch Skills Hub.
...
# ---------------------------------------------------------------------------
# Ov3rwatch centralized index source
# ---------------------------------------------------------------------------
...
def _load_hermes_index() -> Optional[dict]:
    ...
    try:
        ...
        if resp.status_code != 200:
            logger.debug("Ov3rwatch index fetch returned %d", resp.status_code)
            return _load_stale_index_cache()
        data = resp.json()
    except (httpx.HTTPError, json.JSONDecodeError) as e:
        logger.debug("Ov3rwatch index fetch failed: %s", e)
        return _load_stale_index_cache()
...
class Ov3rwatchIndexSource(SkillSource):
    """Skill source backed by the centralized Ov3rwatch Skills Index.
...
def create_source_router(auth: Optional[GitHubAuth] = None) -> List[SkillSource]:
    ...
    sources: List[SkillSource] = [
        ...
        Ov3rwatchIndexSource(auth=auth), # Centralized index (search + resolved install paths)
