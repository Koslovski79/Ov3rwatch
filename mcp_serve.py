"""
Ov3rwatch MCP Server — expose messaging conversations as MCP tools.
...
Plus: channels_list (Ov3rwatch-specific extra)
...
class EventBridge:
    ...
    This is the Ov3rwatch equivalent of OpenClaw's WebSocket gateway bridge.
...
def create_mcp_server(event_bridge: Optional[EventBridge] = None) -> "FastMCP":
    """Create and return the Ov3rwatch MCP server with all tools registered."""
    ...
    mcp = FastMCP(
        "hermes",
        instructions=(
            "Ov3rwatch Agent messaging bridge. Use these tools to interact with "
...
def run_mcp_server(verbose: bool = False) -> None:
    """Start the Ov3rwatch MCP server on stdio."""
    if not _MCP_SERVER_AVAILABLE:
        print(
