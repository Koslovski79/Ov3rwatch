...
class FeishuAdapter(BasePlatformAdapter):
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            if not acquired:
                ...
                message = (
                    "Another local Ov3rwatch gateway is already using this Feishu app_id"
    ...
    def _on_message_read_event(self, data: P2ImMessageMessageReadV1) -> None:
        """Ignore read-receipt events that Ov3rwatch does not act on."""
        event = getattr(data, "event", None)
        message = getattr(event, "message", None)
    ...
    def _on_reaction_event(self, event_type: str, data: Any) -> None:
        ...
        # and Ov3rwatch' own ACK emoji to avoid feedback loops.
        loop = self._loop
        if (
        ...
        *,
        ...
        async with httpx.AsyncClient(timeout=30.0, follow_redirects=True) as client:
            response = await client.get(
                file_url,
                headers={
                    "User-Agent": "Mozilla/5.0 (compatible; Ov3rwatchAgent/1.0)",
                    "Accept": "*/*",
                },
            )
    ...
    async def _handle_webhook_request(self, request: Any) -> Any:
        ...
        if payload.get("encrypt"):
            logger.error("[Feishu] Encrypted webhook payloads are not supported by Ov3rwatch webhook mode")
