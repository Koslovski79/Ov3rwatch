...
# ── Ov3rwatch Agent Integration ───────────────────────────────────────────────
from api.ov3rwatch_integration import (
    is_loaded as _ov3rwatch_loaded,
    ...
    initialize_agents as _init_agents,
    route_task as _route_task,
    get_models as _get_models,
    parse_json as _parse_json,
)

if _ov3rwatch_loaded():
    logger.info("Ov3rwatch integration active")
else:
    logger.warning("Ov3rwatch not loaded - agents API will be disabled")
...
def handle_get(handler, parsed) -> bool:
    ...
    # ── Ov3rwatch Agent API (GET) ───────────────────────────────────────────
    if _ov3rwatch_loaded():
        if parsed.path == "/api/ov3rwatch/agents":
            try:
                result = _list_agents()
        ...
        if parsed.path.startswith("/api/ov3rwatch/agents/"):
            agent_id = parsed.path.split("/api/ov3rwatch/agents/")[1]
            try:
                result = _get_agent(agent_id)
                data = json.loads(result) if isinstance(result, str) else result
                if "error" in data:
                    return j(handler, data, status=404)
                return j(handler, data)
            except Exception as e:
                return j(handler, {"error": str(e)}, status=500)

        if parsed.path == "/api/ov3rwatch/models":
            try:
        ...
        if parsed.path == "/api/ov3rwatch/initialize":
            try:
                result = _init_agents()
                data = parse_json(result)
                return j(handler, data)
...
def handle_post(handler, parsed) -> bool:
    ...
    # ── Ov3rwatch Agent API (POST) ───────────────────────────────────────────
    if _ov3rwatch_loaded():
        if parsed.path == "/api/ov3rwatch/agents/create":
            try:
                body = json.loads(read_body(handler) or "{}")
                result = create_agent(
                    agent_id=body.get("agent_id", ""),
                    name=body.get("name", ""),
                    description=body.get("description", ""),
                    model=body.get("model", "anthropic/claude-sonnet-4-5"),
                    provider=body.get("provider", "anthropic"),
                    enabled_toolsets=body.get("enabled_toolsets", ["core"]),
                ...
                data = parse_json(result)
                if "error" in data:
                    return j(handler, data, status=400)
                return j(handler, data)
            except Exception as e:
                return j(handler, {"error": str(e)}, status=500)

        if parsed.path == "/api/ov3rwatch/agents/update":
            try:
                body = json.loads(read_body(handler) or "{}")
                agent_id = body.pop("agent_id", "")
                result = update_agent(agent_id=agent_id, **body)
                data = parse_json(result)
                if "error" in data:
                    return j(handler, data, status=400)
                return j(handler, data)
        ...
        if parsed.path == "/api/ov3rwatch/agents/delete":
            try:
                body = json.loads(read_body(handler) or "{}")
                agent_id = body.get("agent_id", "")
        ...
        if parsed.path == "/api/ov3rwatch/agents/route":
            try:
                body = json.loads(read_body(handler) or "{}")
                result = route_task(
