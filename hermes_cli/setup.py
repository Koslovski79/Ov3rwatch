"""
Interactive setup wizard for Ov3rwatch Agent.
...
def print_noninteractive_setup_guidance(reason: str | None = None) -> None:
    ...
    print()
    print(color("⚕ Ov3rwatch Setup — Non-interactive mode", Colors.CYAN, Colors.BOLD))
    print()
    if reason:
        print_info(reason)
    print_info("The interactive wizard cannot be used here.")
    print()
    print_info("Configure Ov3rwatch using environment variables or config commands:")
...
def setup_model_provider(config: dict, *, quick: bool = False):
    ...
    if not quick and _supports_same_provider_pool_setup(selected_provider):
        try:
            ...
            print_info(
                "Ov3rwatch can keep multiple credentials for one provider and rotate between"
            )
            print_info(
    ...
    if _vision_needs_setup:
        ...
        print_info("doesn't currently provide one Ov3rwatch can auto-use for vision,")
...
def setup_terminal_backend(config: dict):
    ...
    print_info("Choose where Ov3rwatch runs shell commands and code.")
    ...
    if selected_backend == "local":
        ...
        print_info("  When using Ov3rwatch via Telegram/Discord, this is where")
        print_info(
...
def _setup_telegram():
    ...
    print()
    print_info("📬 Home Channel: where Ov3rwatch delivers cron job results,")
...
def _setup_matrix():
    ...
    if token or get_env_value("MATRIX_PASSWORD"):
        ...
        print()
        print_info("📬 Home Room: where Ov3rwatch delivers cron job results and notifications.")
...
def _setup_mattermost():
    ...
    print()
    print_info("📬 Home Channel: where Ov3rwatch delivers cron job results and notifications.")
...
def _setup_qqbot():
    ...
    print_info("Connects Ov3rwatch to QQ via the Official QQ Bot API (v2).")
...
def _setup_bluebubbles():
    ...
    print_info("Connects Ov3rwatch to iMessage via BlueBubbles — a free, open-source")
...
def setup_gateway(config: dict):
    ...
    print_info("Connect to messaging platforms to chat with Ov3rwatch from anywhere.")
...
# Config values may have different semantics between OpenClaw and Ov3rwatch.
# Instruction/context files (.md) can contain incompatible setup procedures.
_HIGH_IMPACT_KIND_KEYWORDS = {
    "gateway": "⚠ Gateway/messaging — this will configure Ov3rwatch to use your OpenClaw messaging channels",
    "telegram": "⚠ Telegram — this will point Ov3rwatch at your OpenClaw Telegram bot",
    "slack": "⚠ Slack — this will point Ov3rwatch at your OpenClaw Slack workspace",
    "discord": "⚠ Discord — this will point Ov3rwatch at your OpenClaw Discord bot",
    "whatsapp": "⚠ WhatsApp — this will point Ov3rwatch at your OpenClaw WhatsApp connection",
    "config": "⚠ Config values — OpenClaw settings may not map 1:1 to Ov3rwatch equivalents",
...
def _print_migration_preview(report: dict):
    ...
    if conflict_items:
        print(color("  Would overwrite (conflicts with existing Ov3rwatch config):", Colors.YELLOW))
    ...
    if warnings_shown:
        ...
        print()
        print(color("  Note: OpenClaw config values may have different semantics in Ov3rwatch.", Colors.YELLOW))
        print(color("  For example, OpenClaw's tool_call_execution: \"auto\" ≠ Ov3rwatch's yolo mode.", Colors.YELLOW))
...
def _offer_openclaw_migration(hermes_home: Path) -> bool:
    ...
    print_info("Ov3rwatch can preview what would be imported before making any changes.")
    print()
    ...
    # Execute the migration — overwrite=False so existing Ov3rwatch configs are
    ...
    try:
        migrator = mod.Migrator(
            ...
            execute=True,
            workspace_target=None,
            overwrite=False,  # preserve existing Ov3rwatch config
            migrate_secrets=True,
            output_dir=None,
            selected_options=selected,
            preset_name="full",
    ...
    if conflicts:
        print_info(f"Skipped {conflicts} item(s) that already exist in Ov3rwatch (use hermes claw migrate --overwrite to force).")
    if skipped:
...
def run_setup_wizard(args):
    ...
    if section:
        for key, label, func in SETUP_SECTIONS:
            if key == section:
                ...
                print(color(f"│     ⚕ Ov3rwatch Setup — {label:<34s} │", Colors.MAGENTA))
                print(
                    color(
                        "└─────────────────────────────────────────────────────────┘",
                        Colors.MAGENTA,
    ...
    print(
        color(
            "│             ⚕ Ov3rwatch Agent Setup Wizard                │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    print(
        color(
            "│  Let's configure your Ov3rwatch Agent installation.       │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    if is_existing:
        ...
        print_success("You already have Ov3rwatch configured.")
        print()
        ...
        setup_mode = prompt_choice("How would you like to set up Ov3rwatch?", [
...
def _offer_launch_chat():
    ...
    if not chat_argv:
        print_info("Could not relaunch Ov3rwatch automatically. Run 'hermes chat' manually.")
        return
...
def _run_quick_setup(config: dict, hermes_home):
    ...
    if missing_messaging:
        print()
        print_header("Messaging Platforms")
        print_info("Connect Ov3rwatch to messaging apps to chat from anywhere.")
