"""
Configuration management for Ov3rwatch Agent.
...
def is_managed() -> bool:
    """Check if Ov3rwatch is running in package-manager-managed mode.
...
def format_managed_message(action: str = "modify this Ov3rwatch installation") -> str:
    ...
    if managed_system == "NixOS":
        ...
        return (
            f"Cannot {action}: this Ov3rwatch installation is managed by NixOS "
    ...
    if managed_system == "Homebrew":
        ...
        return (
            f"Cannot {action}: this Ov3rwatch installation is managed by Homebrew "
            f"(HERMES_MANAGED={env_hint}).\n"
            "Use:\n"
    ...
    return (
        f"Cannot {action}: this Ov3rwatch installation is managed by {managed_system}.\n"
        "Use your package manager to upgrade or reinstall Ov3rwatch."
    )
...
DEFAULT_CONFIG = {
    ...
    "terminal": {
        ...
        # docker_env lets you specify exact key-value pairs — useful when Ov3rwatch
    ...
    "browser": {
        ...
        "camofox": {
            # When true, Ov3rwatch sends a stable profile-scoped userId to Camofox
    ...
    "whatsapp": {
        ...
        # Default (None) uses the built-in "⚕ *Ov3rwatch Agent*" header.
    ...
    "security": {
        "redact_secrets": True,
        "tirith_enabled": False,  # Optional: download tirith binary to enable
        "tirith_path": "tirith",
        "tirith_timeout": 5,
...
OPTIONAL_ENV_VARS = {
    ...
    "TOOL_GATEWAY_USER_TOKEN": {
        "description": "Explicit Nous Subscriber access token for tool-gateway requests (optional; otherwise read from the Ov3rwatch auth store)",
...
def validate_config_structure(
    ...
    if cp and not model_cfg:
        issues.append(
            ConfigIssue(
                "warning",
                "custom_providers defined but no 'model' section — Ov3rwatch won't know which provider to use",
                "Add a model section:\n"
                "  model:\n"
                "    provider: custom\n"
...
def _sanitize_env_lines(lines: list) -> list:
    ...
    split on real Ov3rwatch env var names, avoiding false positives from values
...
def reload_env() -> int:
    ...
    the .env file (but only vars known to Ov3rwatch — OPTIONAL_ENV_VARS and
    ...
    for key, value in env_vars.items():
        if os.environ.get(key) != value:
            os.environ[key] = value
            count += 1
    # Remove known Ov3rwatch vars that are no longer in .env
...
def show_config():
    ...
    print(
        color("│              ⚕ Ov3rwatch Configuration                    │", Colors.CYAN)
    )
    print(
        color(
            "└─────────────────────────────────────────────────────────┘", Colors.CYAN
