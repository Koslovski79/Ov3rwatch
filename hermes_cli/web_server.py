...
app = FastAPI(title="Ov3rwatch Agent", version=__version__)
...
def _normalize_config_for_web(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    Ov3rwatch supports ``model`` as either a bare string (``"anthropic/claude-sonnet-4"``)
...
def _anthropic_oauth_status() -> Dict[str, Any]:
    ...
    Ov3rwatch resolves Anthropic creds in this order at runtime:
    1. ``~/.hermes/.anthropic_oauth.json`` — Ov3rwatch-managed PKCE flow
    ...
    if hermes_creds and hermes_creds.get("accessToken"):
        return {
            "logged_in": True,
            "source": "hermes_pkce",
            "source_label": f"Ov3rwatch PKCE ({_HERMES_OAUTH_FILE})",
...
def _claude_code_only_status() -> Dict[str, Any]:
    ...
    Claude Code subscription tokens are actively flowing into Ov3rwatch even
    when they also have a separate Ov3rwatch-managed PKCE login.
    """
    try:
...
async def disconnect_oauth_provider(provider_id: str, request: Request):
    ...
    # Anthropic and claude-code clear the same Ov3rwatch-managed PKCE file
...
def _save_anthropic_oauth_creds(access_token: str, refresh_token: str, expires_at_ms: int) -> None:
    """Persist Anthropic PKCE creds to both Ov3rwatch file AND credential pool.
...
def start_server(host: str = "127.0.0.1", port: int = 9119, open_browser: bool = True):
    ...
    print(f"  Ov3rwatch Web UI → http://{host}:{port}")
