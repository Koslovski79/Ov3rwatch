...
Diagnoses issues with Ov3rwatch Agent setup.
"""
...
def run_doctor(args):
    ...
    print(color("│                 🩺 Ov3rwatch Doctor                        │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    if soul_path.exists():
        ...
        check_warn(f"{_DHH}/SOUL.md not found", "(create it to give Ov3rwatch a custom personality)")
        if should_fix:
            soul_path.parent.mkdir(parents=True, exist_ok=True)
            soul_path.write_text(
                "# Ov3rwatch Agent Persona\n\n"
                "<!-- Edit this file to customize how Ov3rwatch communicates. -->\n\n"
                "You are Ov3rwatch, a helpful AI assistant.\n",
                encoding="utf-8",
            )
