...
Ov3rwatch spins up a local Hindsight daemon with built-in PostgreSQL. Requires an LLM API key for memory extraction and synthesis. The daemon starts automatically in the background on first use and stops after 5 minutes of inactivity.
...
| `retain_context` | `conversation between Ov3rwatch Agent and the User` | Context label for retained memories |
