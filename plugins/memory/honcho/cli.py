...
def cmd_sync(args) -> None:
    ...
    Scans all Ov3rwatch profiles and creates host blocks for any that don't
...
def _host_key() -> str:
    """Return the active Honcho host key, derived from the current Ov3rwatch profile."""
...
def cmd_setup(args) -> None:
    ...
    print("\nHoncho memory setup\n" + "─" * 40)
    print("  Honcho gives Ov3rwatch persistent cross-session memory.")
...
def _active_profile_name() -> str:
    """Return the active Ov3rwatch profile name (respects --target-profile override)."""
    if _profile_override:
...
def cmd_peer(args) -> None:
    ...
    if user_name is None and ai_name is None and reasoning is None:
        ...
        print(f"  AI peer:     {ai}")
        print("    Ov3rwatch' identity in Honcho. Seed with 'hermes honcho identity <file>'.")
...
def cmd_tokens(args) -> None:
    ...
    if context is None and dialectic is None:
        ...
        print("    AI-to-AI inference. Ov3rwatch asks Honcho's AI peer a question")
...
def cmd_migrate(args) -> None:
    """Step-by-step migration guide: OpenClaw native memory → Ov3rwatch + Honcho."""
    from pathlib import Path
    ...
    print("\nHoncho migration: OpenClaw native memory → Ov3rwatch\n" + "─" * 50)
    print()
    ...
    if has_key:
        ...
        print("  Honcho is a cloud memory service that gives Ov3rwatch persistent memory")
    ...
    if user_files:
        ...
        print("  (Ov3rwatch calls migrate_memory_files() on first session init.)")
        print()
    ...
    print()
    print("  In Ov3rwatch, they are seeded once into Honcho's AI peer through the")
    ...
    print("    Ov3rwatch:   cloud-backed Honcho peers. Files can stay on disk as source")
    ...
    print("    Ov3rwatch:   Honcho context fetched async at turn end, injected next turn.")
    ...
    print("    Ov3rwatch:   Honcho observes every message and updates representations")
    ...
    print("    Ov3rwatch:   per-session by default — each run gets its own session")
...
def register_cli(subparser) -> None:
    ...
    subs.add_parser(
        "migrate",
        help="Step-by-step migration guide from openclaw-honcho to Ov3rwatch Honcho",
    )
