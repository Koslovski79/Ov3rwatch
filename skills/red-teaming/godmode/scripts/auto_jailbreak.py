...
MODEL_STRATEGIES = {
    ...
    # Nous/Ov3rwatch models — already uncensored, just needs clean prompt
    "hermes": {
        "order": ["prefill_only"],
        "system_templates": {},
    },
...
def _get_current_model() -> tuple:
    """Read current model and provider from Ov3rwatch config.yaml.
    ...
    if winning_strategy:
        ...
        if not dry_run:
            ...
            if verbose:
                ...
                print()
                print("[DONE] Jailbreak locked in. Restart Ov3rwatch for changes to take effect.")
        else:
            if verbose:
...
def undo_jailbreak(verbose=True):
    ...
    if verbose:
        print("[UNDO] Jailbreak removed. Restart Ov3rwatch for changes to take effect.")
