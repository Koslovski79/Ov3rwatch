#!/usr/bin/env python3
"""Google Workspace OAuth2 setup for Ov3rwatch Agent.
...
def _format_missing_scopes(missing_scopes: list[str]) -> str:
    ...
    return (
        ...
        f"{bullets}\n"
        "Run the Google Workspace setup again from this same Ov3rwatch profile to refresh consent."
    )
...
def store_client_secret(path: str):
    """Copy and validate client_secret.json to Ov3rwatch home."""
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace OAuth setup for Ov3rwatch")
